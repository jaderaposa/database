...
$fname = $_GET['fname'];
    ...
    include "connect2.php";
            ...
            if (isset($_POST['submit_form_get'])) {
                ...
                $counname = $_POST['counselorname'];
                $counaddress = $_POST['counseloradd'];
                $counphoneno = $_POST['counselorphoneno'];
                $counspecialty = $_POST['counselorspecialty'];

                $filename = htmlspecialchars (basename($_FILES["image"]["name"]));

                $user_query = "SELECT * FROM counselors WHERE Counselor_ID='$counselor_id'";
                ...
                foreach($user_query_run as $user_row)
                {
                    ...
                    if($filename == NULL)
                    {
                        ...
                        $image_data = $user_row['Counselor_Img'];
                    }
                    else 
                    {
                        // UPDATE WITH NEW IMAGE AND DELETE WITH OLD IMAGE
                        if($img_path = "../../images/".$user_row['Counselor_Img'])
                        {
                            unlink($img_path);
                            $image_data = $filename;
                        }
            ...
            mysqli_query($conn," UPDATE counselors SET Counselor_Name='$counname', Counselor_Address='$counaddress', Counselor_PhoneNo='$counphoneno', Counselor_Specialty='$counspecialty', Counselor_Img='$image_data' WHERE Counselor_ID = '$counselor_id' ");
            
            $result=mysqli_query($conn,"SELECT * FROM counselors");
            ...
            if($result)
            {
                ...
                {
                    // UPDATE WITH EXISTING IMAGE
                    echo "<script>window.alert('Counselor Successfully Updated!'); </script>";
            ...
            {
            $_SESSION['status'] = "Counselor Failed to Update";
